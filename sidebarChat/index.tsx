...
let ChannelSectionStore: any = null;
try {
    const test = findStoreLazy("ChannelSectionStore");
    // Access a known method to force proxy resolution; if it throws, store doesn't exist
    void test?.getSidebarState;
    ChannelSectionStore = test;
} catch {
...
const SidebarView = ErrorBoundary.wrap(() => {
    ...
    const [channelSidebar, guildSidebar] = useStateFromStores(
        [ChannelSectionStore, SelectedChannelStore, ChannelStore].filter(Boolean), () => {
            if (!ChannelSectionStore?.getSidebarState) return [false, false];
